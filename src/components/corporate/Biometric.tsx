...
import { updataData, updateCorporate, updateIndividual, updateSteps } from "../../features/verification/verificationSlice";
...
export const Biometric = () => {
    ...
    const onResetSelfie = async () => {
        ...
        const { payload } = await dispatch(updateCorporate({ data: { selfie: false, fingerPrint: false }, route: 'biometrics', id: data._id }))
        if (payload?.success) {
    ...
    const onToggleForm = () => {
        if (currentCount === 4) {
            dispatch(updateSteps({ current: currentCount + 1, prev: currentCount, completedStep: 6 }))
        } else {
            dispatch(updateSteps({ current: 6, prev: currentCount, completedStep: 6 }))
        }
    ...
    const onSubmitForm = async (e: any) => {
        e.preventDefault()
        const { payload } = await dispatch(updateCorporate({ data: { selfie: true }, route: 'biometrics', id: data._id }))
    ...
    return (
        <div className=" max-w-full  md:max-w-3xl xl:max-w-5xl mx-auto p-5 px-5 md:p-8 md:px-8  bg-white rounded-2xl my-10">
            <div className="flex w-full items-center justify-between">
                <div className="flex items-center gap-1 md:gap-3 font-[400]"><p className="text-md font-[500] text-black">Step 6 -  Biometrics Capture</p>
                    {!completedSteps.includes(6) || !data.bioMetrics?.selfie ? <span className="text-xs text-[#FF3B30] bg-[#FFF0EF] font-[400] p-1 px-2 rounded-3xl">Action Required</span>
                ...
                <button onClick={onToggleForm} className="w-fit">
                    {currentCount === 6 ? <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none">
            ...
            {currentCount === 6 && <div className="min-h-[400px] mt-8">
