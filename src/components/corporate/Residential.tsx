...
export const ResidentialInfo = () => {
    ...
    const onSubmitForm = async (e: any) => {
        e.preventDefault()
        const { payload } = await dispatch(updateCorporate({ data: { ...residentialData, zipCode: Number(residentialData.zipCode) }, route: 'address', id: data._id }))
        if (payload?.success) {
    ...
    const isCompleted = () => {
        return data?.address?.address && data?.address?.city && data?.address?.state && data?.address?.zipCode && data?.address?.landMark
    }
    ...
    return (
        <div className=" max-w-full  md:max-w-3xl xl:max-w-5xl mx-auto p-5 px-5 md:p-8 md:px-8  bg-white rounded-2xl my-10">
            <div className="flex w-full items-center justify-between">
                <div className="flex items-center gap-3 font-[400]"><p className="text-md  text-black font-[500]">Step 2 - Corporate Address</p>
                    {!completedSteps.includes(2) || !isCompleted() ? <span className="text-xs text-[#FF3B30] bg-[#FFF0EF] font-[400] p-1 px-2 rounded-3xl">Action Required</span>
                ...
                <button onClick={onToggleForm} className="w-fit">
                    {currentCount === 2 && isCompleted() ? <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none">
