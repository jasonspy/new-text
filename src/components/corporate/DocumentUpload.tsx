...
import { updataData, updateSteps } from "../../features/verification/verificationSlice";
...
export const DocumentUpload = () => {
    ...
    const onSubmitForm = async (e: any) => {
        e.preventDefault()
        upload ? await uploadFile(upload) : dispatch(updateSteps({ current: currentCount + 1, prev: currentCount, completedStep: 4 }))
    }

    const onReset = (e: any) => {
        setImagePreview('')
        uploadFile(null)
        dispatch(updateSteps({ current: 5, prev: currentCount, completedStep: 4 }))
    }
    const uploadFile = async (file: any) => {
        setIsUploading(true)
        const API_ENDPOINT = `http://159.203.142.44:4080/api/v1/corporate/documents/${data?._id}`;
        ...
        request.onreadystatechange = () => {
            if (request.readyState === 4 && request.status === 200) {
                ...
                if (response.data.documents.identity) {
                    dispatch(updateSteps({ current: currentCount + 1, prev: currentCount, completedStep: 5 }))
                }
                setIsUploading(false)
            }
        };
        formData.append('type', docs.type || 'CAC Certificate')
    ...
    return (
        <div className=" max-w-full  md:max-w-3xl xl:max-w-5xl mx-auto p-5 px-5 md:p-8 md:px-8  bg-white rounded-2xl my-10">
            <div className="flex w-full items-center justify-between">
                <div className="flex items-center gap-3 font-[400]"><p className="text-md text-black font-[500]">Step 5 - Upload Document</p>
                    {completedSteps.includes(5) && imagePreview ? <span className="text-xs text-[#34C759] bg-[#EFFFEF] font-[400] p-1 px-2 rounded-3xl">Completed</span>
                ...
                <button onClick={onToggleForm} className="w-fit">
                    {currentCount === 5 && docs.type && docs.identity ? <svg xmlns="http://www.w3.org/2000/svg" className="" width="20" height="20" viewBox="0 0 24 24" fill="none">
            ...
            {currentCount === 5 && <div className="min-h-[400px] mt-8">
                ...
                <form className="w-full my-5" onSubmit={onSubmitForm}>
                    {!imagePreview && <div className="grid md:grid-cols-2 md:gap-10">
                        <div className="mb-6 md:w-[90%]">
                            ...
                            <div className="mt-1 relative">
                                <select name="upload" id="upload" onChange={(e: any) => setDocs({ ...docs, type: e.target.value })} value={docs?.type} className="block w-full rounded-[12px] border border-[#E8EAED] py-3 px-2.5 placeholder:text-placeholder placeholder:text-placeholder-inactive placeholder:text-sm  sm:text-sm sm:leading-6 outline-none h-[50px] bg-[#ECECEC] border-[#8F8F8F]">
                                    <option value="CAC Certificate" selected>CAC Certificate</option>
                    ...
                    {imagePreview && <div>
                        <div className="w-64 border rounded-lg">
                            <img src={imagePreview} alt="" className="h-28 w-full rounded-t-lg object-cover" />
                            <p className="p-2 text-sm uppercase">{docs?.type || 'CAC Certificate'}</p>
                        </div>
                    </div>}
                    ...
                    {!imagePreview && <div className="my-4">
                        <div className="flex w-full items-center justify-center bg-grey-lighter">
                            <label className="w-full flex flex-col items-center px-4 py-6 bg-[#F5F5F5] text-blue rounded-lg  tracking-wide p-8 md:py-20 cursor-pointer">
                                ...
                                <input type='file' className="hidden" onDragEnter={handleFileUpload} onInput={handleFileUpload} />
                            </label>
                        </div>
                    </div>}
